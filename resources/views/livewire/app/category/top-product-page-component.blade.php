        ...
        <div class="my-container">
            <div class="category_product_title">
                ...
                <div class="d-flex align-items-center justify-content-between flex-wrap-wrap g-sm">
                    <h4>75,785 itemd found in Discount Products</h4>
                ...
                <div class="tab_item" style="display:block;">
                    <div class="new_arrival_grid">
                        ...
                        <div class="product_item product_item_text">
                            <a href="{{ route('front.productDetails', ['slug' => $topProduct->slug]) }}" class="product_img hover_color">
                                <img src="{{ $topProduct->thumbnail }}" alt="{{ $topProduct->name }}" onerror="this.onerror=null;this.src='{{ asset('assets/images/placeholderbg.png') }}';" />
                            </a>
                            <div class="product_content">
                                <h3>
                                    <a href="{{ route('front.productDetails', ['slug' => $topProduct->slug]) }}">{{ $topProduct->name }}</a>
                                </h3>
                               
                                <div class="product_ratting_area product_review_img_area d-flex align-items-center g-sm"
                                style="gap: 7px 4px;">
                                {!! avarage_review($topProduct->id) !!} <span>({{
                                    product_review($topProduct->id) }})</span>
                            </div>
                                <div class="prodcut_price d-flex align-items-center flex-wrap-wrap">
                                    <h5>TND {{ discountPrice($topProduct->id) }}</h5>
                                    <h5><del>TND {{ $topProduct->unit_price }}</del></h5>
                                </div>
                            </div>
                        </div>
                        @endforeach
                    </div>
                    {{ $topProducts->links('front-pagination-links') }}
                </div>
            </div>
        </div>
    </section>

            ...
            <div class="tab_content_area">
                <div class="tab_item" style="display:block;">
                    <div class="new_arrival_grid">
                        @foreach ($apparelproducts as $apparelproduct)
                        <div class="product_item product_item_text">
                            <a href="{{ route('front.productDetails', ['slug' => $apparelproduct->slug]) }}" class="product_img hover_color">
                                <img src="{{ $apparelproduct->thumbnail }}" alt="{{ $apparelproduct->name }}" onerror="this.onerror=null;this.src='{{ asset('assets/images/placeholderbg.png') }}';" />
                            </a>
                            <div class="product_content">
                                <h3>
                                    <a href="{{ route('front.productDetails', ['slug' => $apparelproduct->slug]) }}">{{ $apparelproduct->name }}</a>
                                </h3>
                               
                                <div class="product_ratting_area product_review_img_area d-flex align-items-center g-sm"
                                style="gap: 7px 4px;">
                                {!! avarage_review($apparelproduct->id) !!} <span>({{
                    ...
                    {{ $apparelproducts->links('front-pagination-links') }}
                </div>
            </div>
        </div>
    </section>
